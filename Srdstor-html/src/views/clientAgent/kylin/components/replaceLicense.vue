<template>
  <el-dialog
    ...
    title="替换许可"
    width="600px">
    <el-form v-loading="loading" ref="replaceLicense" :rules="rules" :model="form" label-width="100px">
      ...
      <el-form-item label-width="0" style="text-align: right;">
        <el-button type="primary" @click.native="submitForm('replaceLicense')">注 册</el-button>
...
<script>
import { getMcode, replaceLicense, getLicenseInfo } from '@/api/SRDstor/clientAgent'
...
export default {
  ...
  methods: {
    handleClose() {
      this.$refs['replaceLicense'].resetFields()
      ...
      getLicenseInfo({ cmdcode: 82 }, `${self.currentRow.ip}:8888`).then(resp => {
        if (resp.retcode === 0) {
          if (resp.data.status === 0) {
            self.$message({ type: 'success', message: `替换成功` })
            self.$emit('updateData', false)
            self.handleClose()
          } else {
            self.$emit('updateData', false)
            self.$message({ type: 'error', message: `替换失败` })
          }
        } else {
          self.$message({ type: 'error', message: `替换失败` })
        }
