  ...
  <el-dialog
    ...
    <el-form v-loading="loading" ref="updateiSCSITargetForm" :model="form" :rules="rules" label-width="120px">
      ...
      <el-form-item label-width="0" style="text-align: right;">
        <el-button type="primary" @click="submitForm('updateiSCSITargetForm')">创 建</el-button>
    ...
    submitForm(formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // params.name = 'iqn.1993-01.com.test01:' + params.name
          createiSCSITarget(params)
            .then((resp) => {
              ...
              if (resp.rc === 0) {
                ...
                _this.$message({ type: 'success', message: '创建iSCSITarget成功' })
              } else {
                _this.$message({ type: 'error', message: '创建iSCSITarget失败' })
              }
            })
            .catch(() => {
              _this.loading = false
