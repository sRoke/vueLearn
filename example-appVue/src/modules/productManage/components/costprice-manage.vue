<template>
    <buModal :title="title" :writeshow="showWrite" :addshow="showAdd" :confshow="showConf" @close="close" @back="close" @conf="conf">
        <buInput v-for="(item,index) in scopeUnit" :key="index" :title="'商品成本（' + item.name + '）'" :disable="disable" placeholder="输入商品成本" type="number" v-model="scopePrice[item.type]"></buInput>
    </buModal>
</template>
<script>
