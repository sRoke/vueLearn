<template>
  <div id="customerList">
    <div class="m-b-20 ovf-hd">
      ...
      <div class='fr m-l-10'>
        <reset-btn :reset="handleClearFilter"></reset-btn>
      </div>
      <div class="fl"
           v-if="isEdit">
      ...
      <div class="fr publicClient"
           v-if="isView">
        <el-select v-model="business_category"
                   placeholder="请选择营业品类"
                   @change="select()"
                   class="selects m-r-10">
          <el-option v-for="item in sellType"
                     :label="item.name"
                     :value="item.id">
          </el-option>
        </el-select>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column label="营业品类"
                       ...
                       column-key="business_category">
      </el-table-column>
      <el-table-column label="跟进人员"
                       prop="follow_user"
                       :filter-multiple=false
                       :filters="followPeople"
                       column-key="follow_user">
      </el-table-column>
      ...
      <el-table-column label="跟进状态"
                       prop="follow_status"
                       :filter-multiple=false
                       :filters="[{ text: '全部', value: '' }, { text: '未联系', value: '2' }, { text: '已跟进', value: '1' },{text: '需回访',value: '3'},{text: '无效',value: '2'}]"
                       column-key="follow_status">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    select() { // 通过路由的参数保存页面的状态
      ...
      let query = {
        ...
        level: self.level,
        province: self.proValue,
        city: self.cityValue,
        business_category: self.business_category,
        account: self.account,
        follow_user: self.follow_user
      }
      router.push({ path: '/data/service/demanderList', query: query })
      // this.getCustomerList()
    },
    init() { // 根据页面的状态获取页面数据
      ...
      if (self.$route.fullPath == '/data/service/demanderList') {
        ...
        self.cityValue = ''
        self.account = ''
        self.business_category = null
        self.follow_user = null
      } else {
        ...
        self.account = self.$route.query.account
        self.business_category = self.$route.query.business_category
        self.follow_user = self.$route.query.follow_user
      }
      self.path = self.$route.fullPath
      // console.log(self.path)
      self.getCustomerList()
    },
    handleSizeChange(val) { // 改变页面数据的条数
      this.pageSize = val
    ...
    applyFilter(filter) { // 根据客户级别的条件或跟进状态的条件刷选
      ...
      if (filter.level) {
        if (filter.level.length == 0) {
          self.level = ''
        } else if (filter.level.length > 0) {
          self.level = filter.level[0]
        }
      }
      console.log(filter.follow_user)
      if (filter.follow_user) {
        if (filter.follow_user.length == 0) {
          self.follow_user = ''
        } else if (filter.follow_user.length > 0) {
          this.follow_user = filter.follow_user[0]
        }
      }
      if (filter.follow_status) {
        if (filter.follow_status.length == 0) {
          self.status = ''
        } else if (filter.follow_status.length > 0) {
          self.status = filter.follow_status[0]
        }
      }
    ...
    getCustomerList() { // 获取客户列表
      ...
      console.log('需求方列表')
      var self = this
      console.log(self.business_category)
      ...
      console.log(data)
      if (self.isView) {
        self.apiGet('demand_user', data).then((res) => {
    ...
    customerData(res) { // 对获取的客户列表的信息进行处理
      ...
      if (res.code != 200) {
        ...
        self.currentPage = parseInt(res.data.current_page)
        self.dataCount = res.data.total_count
        if (res.data.list) {
          for (var j = 0; j < res.data.list.length; j++) {
            switch (res.data.list[j].level) {
              case 1: self.tableData[j].level = 'A热门客户'
                break
              case 2: self.tableData[j].level = 'B普通客户'
                break
              case 3: self.tableData[j].level = 'C潜在客户'
                break
              case 4: self.tableData[j].level = 'D无法沟通'
                break
            }
  ...
  created() {
    var self = this
    this.apiGet('product_types').then(res => { // 获取产品类别
      if (res.code == 200) {
        console.log(res.data)
        this.sellType = res.data
      }
      console.log(this.sellTypes)
    })
    this.apiGet('customers/customer_follow_list').then(res => { // 获取跟进人员
      if (res.code == 200) {
        console.log(res.data)
        for (var i = 0; i < res.data.length; i++) {
          ...
          this.followPeople.push(x)
        }
        console.log(this.followPeople)
      }
    })
    if (Lockr.get('authList')) { // 判断权限
