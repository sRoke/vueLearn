  ...
  <div id='serviceList'>
    ...
    <div class="m-b-20 ovf-hd">
      <div class="fr w-400">
        ...
        <el-col :span='4' class='m-l-20' v-if='service_edit' style="width:50px">
        <div>
      ...
      <div class="fr service">
        <el-select v-model="business_category" placeholder="请选择营业品类" @change="search()" class="selects m-r-10">
          <el-option
        ...
        <el-select v-model="proValue" placeholder="请选择省" @change="search()" class="selects">
          <el-option
        ...
        <el-select v-model="cityValue" placeholder="请选择市" class="m-l-10 selects" @change="search()">
          <el-option
        ...
        <el-input placeholder="请输入账号" v-model="account" class="w-200">
          <el-button slot="append" icon="search" @click.prevent = "search()"></el-button>
        </el-input>
      </div>
    ...
    <el-table
        ...
        <el-table-column
          ...
          width="120"
          :filter-multiple=false
          :filters="[{ text: '跟进状态', value: '' }, { text: '未联系', value: '2' }, { text: '已跟进', value: '1' },{text: '需回访',value: '3'},{text: '无效',value: '2'}]" 
        ...
        <el-table-column label="最新跟进时间" prop="follow_time">
      </el-table-column>
        <!-- <el-table-column
          prop="follow"
          label="跟进记录"
...
export default{
  ...
  methods: {
    ...
    applyFilter(filter) { // 根据跟进状态和跟进人员等的条件刷选
      var self = this
      // alert(_g.j2s(filter))
      if (filter.is_auth) {
        console.log(filter.is_auth)
        if (filter.is_auth.length == 0) {
          this.is_auth = 'all'
        } else {
          this.is_auth = filter.is_auth[0]
        }
      }
    ...
    sortData(list) {   // 获取到数据后，整理
      ...
      for (var i = 0; i < this.tableData.length; i++) {
        ...
        this.tableData[i].position = this.tableData[i].province + this.tableData[i].city
  ...
  created() {
    ...
    this.apiGet('customers/customer_follow_list').then(res => {
      if (res.code == 200) {
        // console.log(res.data)
        // for (var i = 0; i < res.data.length; i++) {
        //   let x = {
