  ...
  export default{
    ...
    methods: {
      ...
      select() { // 通过路由的参数保存页面的状态
        ...
        router.push({ name: 'deDetail', params: { id: self.cus_id }, query: { index: 'second', page: self.currentPage, keyword: self.keywords, status: self.status, coo_model: self.coo_model }})
      },
      init() { // 根据页面状态获取需求记录列表
      ...
      getData() { // 获取需求记录列表
        ...
        self.apiGet('demand_user/' + self.cus_id + '/requires', data).then((res) => {
