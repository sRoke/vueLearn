...
export default {
  ...
  methods: {
    ...
    choosed() { // 打开选择所属公司的对话框
      this.$refs.companyChoose.open()
    },
    goback() {
      router.go(-1)
    },
    uploadSuccess1 (res, file, fileList1) { // 缩略图上传成功
      this.pic1 = window.imgUrl + res.data
      this.form.imgUrl = res.data
    },
    uploadSuccess2 (res, file, fileList) { // 轮播图上传成功
      this.ifOne = true
      console.log(file)
      console.log(res.data)
      let url = {}
    ...
    uploadSuccess3(res, file, fileList3) { // 详情页图片上传成功
      console.log(file)
      console.log(res.data)
      let urls = {}
  ...
  mounted() {
    ...
    this.editor = new Wangeditor('div3') // 富文本编辑器实例化
