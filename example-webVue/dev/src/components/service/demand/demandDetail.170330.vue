<template>
  <div id="main-content" v-cloak>
    ...
    <div v-if="isEdit" class="gray-14 fz-14">
      <el-row :gutter="20">
      ...
      <el-row :gutter="20" class="m-t-10">
        <el-col :span="22" :offset="1">
          <div>
            ...
            <div class="fl">
              <div class="detail-label bor-ra-5 fl m-t-10" v-for="la in detail.label">{{la.label_name}}</div>
            </div>
          </div>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20" class="m-t-20">
        <el-col :span="22" :offset="1" class="p-b-20">
          <div>
            ...
            <div class="fl w-80P">
              ...
              <div class="fl w-80P" v-if="showBonus">
      ...
      <div style="width: 100%" class="tx-c m-t-10" v-show="allEdit">
        <el-button type="primary" @click="showEdit()" v-show="isEditBtn">编辑需求</el-button>
      </div>
    </div>
    <div v-else>
      <el-form ref="form" :model="up" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          <el-col :span="10" :offset="1">
            <el-form-item label="需求方:" prop="demander">
              <el-input v-model.trim="up.demander" :disabled='true' class="h-40 w-200"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="10" :offset="2">
        ...
        <el-row :gutter="20">
          <el-col :span="10" :offset="1">
            <el-form-item label="网站账号:" prop="user_phone">
              <el-input v-model.trim="up.user_phone" class="h-40 w-200"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="10" :offset="2">
              ...
              <el-row type='flex' justify='start'>
                ...
                <div>
                  <el-input class='m-l-15' v-model="up.address" placeholder="请输入详细地址"></el-input>
                </div>
              </el-row>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="10" :offset="1">
            <el-form-item label="目标费用：">
              <el-input type="number" v-model="up.budget_min" class="h-40 w-80"></el-input> -
              <el-input type="number" v-model="up.budget_max" class="h-40 w-80"></el-input> 元
            </el-form-item>
          </el-col>
          <el-col :span="10" :offset="2">
              <!-- el-input v-model.trim="up.address" class="h-40 w-200"></el-input> -->
              ...
              <el-row type='flex' justify='start'>
                <el-select placeholder="请选择产品类别" v-model="area" @change="areaChange">
                  <el-option
        ...
        <el-row :gutter="20">
          <el-col :span="22" :offset="1">
            <el-form-item label="需求标题:" prop="title">
              <el-input v-model.trim="up.title" class="h-40 w-200"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="22" :offset="1">
            <el-form-item label="需求标签:">
              <div class="fl w-400 m-t-5">
                <div class="bor-ra-5 edit_label fl" v-for="la in detail.label">{{la.label_name}}</div>
                <el-button type="primary" size="small" class="fl m-l-10" @click="openTag(0)">选择</el-button>
              </div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="22" :offset="1">
            <el-form-item label="需求备注:">
              <el-input type="textarea" v-model.trim="up.remark" :rows="4" class="w-400"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="22" :offset="1">
            <el-form-item label="合作模式:">
              <el-radio-group v-model="up.coo_model" :change="changeModel()">
                <el-radio label="1">项目制</el-radio>
                <el-radio label="2" :disabled="!hasAppDesign">销量提成（专指外观设计）</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        </el-row>
            ...
            <div class="w-300 fl">
              <el-form-item label="设计方案:">
                <el-checkbox-group v-model="up.design_way">
                  <el-checkbox label="10" class="w-120 m-l-15" name="design_way">外观设计及数据</el-checkbox>
                  <el-checkbox label="20" class="w-120" name="design_way" :disabled="!hasAppDesign">外观手板</el-checkbox>
                  <el-checkbox label="30" class="w-120 m-t-40" v-bind:class="{'m-t-180': showBonus}" name="design_way">内部结构设计</el-checkbox>
                  <el-checkbox label="40" class="w-120" v-bind:disabled="!hasStructDesign" name="design_way">功能手板</el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </div>
          </el-col>
          <el-col :span="15" :offset="1">
            <div class="fl form-bg" v-if="hasAppDesign">
              <div class="fl" style="width: 100%">
                <div class="fl">
                  <el-form-item label="预付款" prop="outward_pre_pay">
                    <el-input v-model.trim="up.outward_pre_pay" class="h-40 w-80"></el-input> 元
                  </el-form-item>
                </div>
                <div class="fl" v-show="!showBonus">
                  <el-form-item label="尾款" prop="outward_balance_amount">
                    <el-input v-model.trim="up.outward_balance_amount" class="h-40 w-80"></el-input> 元
                  </el-form-item>
                </div>
                <div class="fl" v-show="showBonus">
                ...
                <div class="fl" v-show="showBonus">
                  <el-form-item label="提成时长">
                    <el-input v-model.trim="up.bonus_duration" class="h-40 w-80"></el-input> 个月
                  </el-form-item>
                </div>
              </div>
              <div class="fl" v-show="showBonus">
                <div class="fl w-600">
                  <el-form-item label="提成方案">
                    <div class="fl">
                      <span class="tx-c w-80" style="display: inline-block;">0</span> 至
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus1.end_count"></el-input> 台(单位)，提成费为
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus1.amount"></el-input> 元/台(单位)
                    </div>
                    <div class="fl">
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus2.start_count"></el-input> 至
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus2.end_count"></el-input> 台(单位)，提成费为
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus2.amount"></el-input> 元/台(单位)
                    </div>
                    <div class="fl">
                      <el-input class="h-40 w-80" type="number" :min="1" v-model.trim="bonus3.start_count"></el-input> 台(单位)以上，提成费为
                      <el-input class="h-40 w-80" v-model.trim="bonus3.amount"></el-input> 元/台(单位)
                    </div>
                  </el-form-item>
                </div>
              </div>
            ...
            <div class="fl m-t-20 form-bg" v-if="hasStructDesign">
              <div class="fl">
                <el-form-item label="预付款" prop="in_pre_pay">
                  <el-input :min="1" v-model.trim="in_pre_pay" class="h-40 w-80"></el-input> 元
                </el-form-item>
              </div>
              <div class="fl">
                <el-form-item label="尾款" prop="in_balance_amount">
                  <el-input :min="1" v-model.trim="in_balance_amount" class="h-40 w-80"></el-input> 元
                </el-form-item>
              </div>
            </div>
          </el-col>
      ...
      <div style="width: 100%" class="tx-c m-t-30">
        <el-button type="primary" @click="upData()" :loading="flagTrans('upData')">保存需求</el-button>
      </div>
    </div>
    <el-row :gutter="20" class="m-t-30">
      <el-col :span="22" :offset="1" class="bg-gray bor-ra-5 p-all-15">
        <div>
          <div class="fl m-l-30 status-sel w-200 m-tb-8">{{detail.status_name}} <span class="fz-16" v-show="(showDeadline) && (deadline == 0)">(已截止)</span></div>
          <div class="fl m-l-30" v-show="allEdit">
            <el-button size="large" @click="upStatus(demandStatus.btn1.status)" v-if="demandStatus.btn1.show" :loading="flagTrans(demandStatus.btn1.status)">{{demandStatus.btn1.name}}</el-button>
            <el-button type="primary" @click="upStatus(demandStatus.btn2.status)" size="large" v-if="demandStatus.btn2.show" :loading="flagTrans(demandStatus.btn2.status)">{{demandStatus.btn2.name}}</el-button>
          </div>
          <div class="fr m-r-30 m-tb-8 status-sel" v-show="(showDeadline) && (deadline > 0)">倒计时：<span>{{timeCalc}}</span></div>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="20" class="m-t-30" v-show="showSerout" v-if="hasAppDesign">
      <el-col :span="22" :offset="1">
            ...
            <el-table-column
              ...
              <template scope="scope">
                ...
                <el-button
                  ...
                  v-show="(allEdit) && (scope.row.status == 60)">
                  重新邀请
                </el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-button class="fl m-l-10" @click="openSer(0, 0)" v-show="allEdit">添加服务方</el-button>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="20" class="m-t-30" v-show="showSerin" v-if="hasStructDesign">
      <el-col :span="22" :offset="1">
        <div class="service-table">
          ...
          <el-button class="fl m-l-10" @click="openSer(1, 0)" v-show="allEdit">添加服务方</el-button>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="20" class="m-t-30" v-show="showMatch">
      <el-col :span="7" :offset="1" v-if="hasAppDesign">
        <div class="fl gray-14">
          <span class="labelTitle">外观设计：</span>
          <el-button type="text">{{detail.out_servicer}}</el-button>
        </div>
      </el-col>
      <el-col :span="4" v-if="hasAppDesign">
        <el-button type="primary" size="small" @click="openTag(1, 0)">匹配记录</el-button>
        <el-button type="primary" size="small" @click="openSer(0, 1)" v-show="(detail.out_orderstatus == 101)">重新选择</el-button>
      </el-col>
      <el-col :span="7" v-show="detail.in_servicer != ''">
        <div class="fl gray-14">
          <span class="labelTitle">内部结构：</span>
          <el-button type="text">{{detail.in_servicer}}</el-button>
        </div>
      </el-col>
      <el-col :span="4" v-show="detail.in_servicer != ''">
        <el-button type="primary" size="small" @click="openTag(1, 1)">匹配记录</el-button>
        <el-button type="primary" size="small" @click="openSer(1, 1)" v-show="(detail.in_orderstatus == 101)">重新选择</el-button>
      </el-col>
    </el-row>
    <tabInfo ref="tabInfo" :isOne="isOne" :status="tabData" :info="tabMsg" :conId="conId"></tabInfo>
    <editInfo :title="up.title" :label="labelList" ref="editInfo"></editInfo>
    <servicer :isIn="isInSer" :isNewSer="isNewSer" :inData="inService" :outData="outService" ref="servicer"></servicer>
    <recordInfo ref="recordInfo" :type="reType"></recordInfo>
  </div>
</template>
<style type="text/css">
  .gray-14 .labelTitle {
      ...
      getData() {
        this.apiGet('requires/' + this.id).then((res) => {
          if (res.code == 200) {
            ...
            // 判断合作模式
            if (res.data.coo_model == 1) {
              this.showBonus = false
            } else if (res.data.coo_model == 2) {
              this.showBonus = true
            }
            // 判断设计方案的选定
          ...
          if (pass) {
            ...
            this.apiPut('requires/' + this.id + '?action_type=updateInfo', '', this.up).then((res) => {
              ...
              if (res.code == 200) {
                ...
                setTimeout(() => {
                  this.getData()
                  this.postArr.splice(_.indexOf(this.postArr, 'upData'), 1)
                }, 300)
              } else {
                this.postArr.splice(_.indexOf(this.postArr, 'upData'), 1)
                _g.dealError(this, res)
