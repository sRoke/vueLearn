<template>
  <div class="gray-14 fz-14">
    ...
    <el-row :gutter="20" class="m-t-20">
        ...
        <el-col :span="10" :offset="2">
          <div>
            <span class="labelTitle">产品类别：</span>
            <span v-for="item in category">{{ item }}</span>
          </div>
        </el-col>
      </el-row>
    <el-row :gutter="20" class="m-t-20">
      ...
      <el-col :span="22" :offset="1" class="bg-gray bor-ra-5 p-all-15">
        <div>
          ...
          <div class="fl m-l-30 m-t-20" v-show="allEdit">
            ...
            <el-button type="primary" @click="upStatus(demandStatus.status)" size="large" :loading="flagTrans(demandStatus.status)" v-if="demandStatus.btnShow">{{demandStatus.btnName}}</el-button>
          </div>
...
<script>
  ...
  export default {
        ...
        if (status == 100) {
          ...
          this.apiPut('orders/' + this.id + '/contracts/' + this.baseData.contract_id, '', { action_type: 'reInitiate' }).then((res) => {
            if (res.code == 200) {
              ...
              this.postArr.splice(_.indexOf(this.postArr, status), 1)
              _g.dealError(this, res)
            }
          }, (res) => {
            this.postArr.splice(_.indexOf(this.postArr, status), 1)
    ...
    computed: {
      ...
      category() {
        let categories = []
        this.detail.area ? categories.push(this.detail.area) : null
        return [..._g.formatCategory(categories)][0]
      }
    },
    components: {
      tabInfo,
