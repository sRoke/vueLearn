        ...
        <div class="fr w-250"
          ...
          <el-input placeholder="请输入公司、联系人、手机号"
            ...
            <el-button slot="append"
                       icon="search"
                       @click.prevent="setQuery({ keyword })"></el-button>
          </el-input>
        </div>
      ...
      <el-table :data="tableData"
        ...
        <el-table-column label="跟进人员"
          ...
          <template scope="scope">
            <span>{{ formatFollow(scope.row.followPeople) }}</span>
          </template>
        </el-table-column>
...
import routerQeury from 'assets/js/routerQuery'
...
export default {
  ...
  watch: {
    province(val, oldVal) {  // 监听省份的选择，以更新城市选项
      this.cityOptions = address['city'][val]
      this.setQuery({ province: val })
    },
    '$route.query'(to, from) { // 监听路由改变
      this.firstLoad ? this.init() : null
    }
  },
  methods: {
    clear(selection, row) {
    ...
    handleSelectionChange(val) { // 批量选择
      ...
      _(val).forEach((res, key) => {
        ...
        arr = arr.concat(res.followOptions || [])
      })
      this.followOptionss = arr
    },
    select() { // 通过路由的参数保存页面的状态
      // const { keyword, currentPage, province, cityValue, level, follow, follow, pageSize, register, category } = this
      // let query = {}
    ...
    init(params = null) { // 根据页面的状态获取页面数据
      ...
      !this.getQuery.page ? this.setQuery({ page: 1 }) : null
      !this.getQuery.pagesize ? this.setQuery({ pagesize: 15 }) : null
      this.getQuery.follow ? this.follow = ~~this.getQuery.follow : null
      this.getQuery.province ? this.province = this.getQuery.province : null
      this.getQuery.city ? this.city = this.getQuery.city : null
    ...
    getCustomerList() { // 获取客户列表
      ...
      if (this.canView) {
        let demandOrService = this.demandOrService || null
        let params = { isAuth: 'all', demandOrService, ...this.getQuery }
        // let params = {
        //   isAuth: 'all',
    ...
    formatFollow(data) { // 格式化客户管理
      let text = ''
      if (data && data.length > 0) {
        text = _.map(data, 'name').join('、')
      } else {
        text = '未分配人员'
      }
      return text
    ...
    formatLevel(data) {
      let level = '未选择'
      if (data) {
        let levelList = _.map(this.levelOptions, 'name')
    ...
    download() { // 下载模板
      ...
      let params = {
        isDemand: this.demandOrService === 1
      }
      ...
      if (this.canView) {
        this.apiGet('customers/customer_follow_list').then((res) => {
          if (res.code != 200) {
            ...
            this.followOptions = res.data
            this.followOptions.unshift({ name: '全部', id: '' })
            this.followOptions.push({ name: '未分配人员', id: -1 })
          }
        })
      } else {
        return
