    ...
    <div class='base-info-cont'>
      ...
      <el-row v-if="!isEdit">
        <el-button v-if="canEdit"
                   ...
                   type="primary"
                   class='m-l-10'
                   @click.prevent="uploadFile()">添加</el-button>
      </el-row>
      ...
      <div style="margin-top: 15px;"
           class='m-b-10 m-l-10'>
        <span>公司认证：</span>
        <el-checkbox v-if="canEdit"
                     v-model="checked"
                     :checked="checked"
                     :disabled="isEdit">已认证</el-checkbox>
        <span v-else>{{ authLabel }}</span>
      </div>
    </div>
    <div style="margin-top: 15px;"
         class='base-info-cont'>
      ...
      <el-form ref="form"
        ...
        <el-row>
          ...
          <el-col :span="14">
            <el-form-item label="跟进人员：">
              <el-col v-if="canfollowAndRelease && canEdit"
                      :span="1"
                      class="pointer"
                      style="color: #66CCFF;">
                <el-button type="text"
                           @click="selectStaff()">选择</el-button>
              </el-col>
              <el-col :span="6"
                ...
                <el-form-item :label="item.label"
                  ...
                  <el-tag type="gray"
                          v-show="item.name"
                          :closable="is_saleClose"
                          @close="closeTag(item)">{{ item.name }}</el-tag>
                </el-form-item>
              </el-col>
              <el-col :span="2"
                      class="release">
                <el-button v-if="canfollowAndRelease && canEdit"
                           type="text"
                           v-show="followPeople.length > 0"
                           @click.prevent="release()">{{ cancel_release }}</el-button>
              </el-col>
              <staff ref="staff"
        ...
        <el-row>
          ...
          <el-col :span="21">
            <el-table :data="sturdyData"
                      class='m-b-10'
                      border>
              <el-table-column prop="content"
                               label="跟进记录"></el-table-column>
              <el-table-column prop="create_time"
                               label="添加时间"
...
export default {
  ...
  methods: {
    ...
    addRecord() { // 添加跟进说明
      ...
      if (self.text) {
        var temp = {
          ...
          type: true,
          return_visit: moment(self.time).format('YYYY-MM-DD HH:mm:ss')
        }
    ...
    edit() { // 保存修改信息
      ...
      for (let item of self.followPeople) {
        let isExist = _.indexOf(self.follow_people, item.id)
        isExist === -1 ? self.follow_people.push(item.id) : null
      }
