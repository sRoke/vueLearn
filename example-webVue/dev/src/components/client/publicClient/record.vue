<template>
  <div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="mark_status"
                       label="匹配状态">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getData() { // 获取需求记录列表
      ...
      self.apiGet(this.requestUrl, data).then((res) => {
  ...
  computed: {
    requestUrl() {
      ...
      return this.$route.query.type == 1 ? `demand_user/${dsu_id}/requires` : `match_records/service/${dsu_id}`
    }
  },
  components: {
    'reset-btn': resetBtn
