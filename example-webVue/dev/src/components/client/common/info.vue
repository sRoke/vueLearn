  ...
  <div>
    <div class="customerData"
        ...
        <el-form :inline="true"
          ...
          <el-row>
            ...
            <el-form-item label="公司电话："
                          v-if="canEdit || onlyView">
              <el-input class="w-500"
                        v-if="canEdit "
                        v-model="form.mobile"
                        :disabled="isEdit"></el-input>
      ...
      <el-card class="box-card contact-card m-b-20">
        ...
        <div class="tb-bottom"
             v-if="canAddCategory && canEdit">
          <el-button type="text"
                     icon="plus"
                     @click="openCategoryDialog()">添加</el-button>
        </div>
      </el-card>
      <!-- 营业品类模板 -->
    ...
    <div class="fixed-footer">
      ...
      <el-button type="success"
                 @click="unRelative()"
                 v-if="form.dsu_id && canUnRelative && canEdit">取消关联</el-button>
...
export default {
  ...
  methods: {
    ...
    getCustomerInfo() { // 获取客户信息
      ...
      self.apiGet('customers/' + self.cus_id).then((res) => {
        if (res.code != 200) {
          ...
          self.form.business_category = res.data.category
          self.categoryList = [..._g.formatCategory(res.data.category)]
    ...
    edit() { // 保存修改信息
      ...
      let followRecord = _(this.new__follow_record).map(item => {
        return {
          ...
          level: item.level ? _.find(this.levelOptions, { label: item.level }).value : null,
      ...
      if (this.form.business_category.length > 0) {
        data.category = [...this.form.business_category]
      }
      if (data.status !== 2) {
    ...
    editCategory(data) { // 编辑品类
      // 判断一级品类是否重复
      let arr = this.form.business_category.filter((item, index) => index !== this.categoryIndex)
      let ids = _.map(arr, 'id')
      let exist = _.includes(ids, data.id)
      if (exist) return this.$message.error('一级品类不可重复')
      // 编辑成功
    ...
    fetchLinks() {
      let data = {
        ...
        condition: { ...this.$route.query, demandOrService: this.$route.query.type }
      }
      this.apiPost('customers/links', data).then(res => {
        if (res.code === 200) {
          this.link.last = res.data.last
          this.link.next = res.data.next
        }
      })
    },
    goPrev() { // 跳转到上一家
      let params = { id: this.link.last }
  ...
  computed: {
    ...
    canEdit() {
      ...
      // let publicClient = this.show == null || this.show['publicClient_edit']
      // let myClient = this.show == null || this.show['myClient_edit']
      // return publicClient || myClient
    },
    onlyView() {
      return _.includes(this.$route.name, 'Detail') && (this.show == null || this.show['myClient_edit']) || (this.show == null || this.show['publicClient_edit'])
    },
    canUnRelative() {
      return this.show == null || this.show['customers_delRelation']
    },
    hasPrev() {
