...
export default {
  ...
  methods: {
    ...
    getCustomerInfo() { // 获取客户信息
      this.toggleLoading()
      let { userid, dsu_id } = this.$route.query
      let id = userid || dsu_id
      let url = this.demandOrService === 1 ? 'demand_user/' : 'service_user/'
      this.apiGet(url + id).then((res) => {
        this.toggleLoading()
        if (res.code != 200) {
