...
export default {
  ...
  methods: {
    ...
    openEdit(row) {
      console.log(row)
      location.hash = '/service/demand/historydetail/' + row.id
    },
    getStatus() {
      this.apiGet('requires/status_list?type=1').then((res) => {
        console.log(res)
        if (res.code == 200) {
    ...
    newPage(num) {
      ...
      router.push({
        path: '/service/demand/historylist',
        query: {
          page: this.currentPage,
