          ...
          <el-table
            ...
            <el-table-column
              ...
              <template scope="scope">
                ...
                <span v-else-if="scope.$index==2">{{scope.row.start_count}}以上</span>
      ...
      <div class="con-text fl m-t-20">
        ...
        <div class="fl m-l-10 w-620">
          <el-form-item prop="design" class="contractMsg">
            <el-checkbox-group v-model="contract.design">
              <el-checkbox disabled class="m-w-65 m-t-5" label="10">外观设计</el-checkbox>
              <el-checkbox disabled class="m-w-65 m-t-5" label="30">结构设计</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </div>
      </div>
      ...
      <div class="m-t-40 m-b-20 tx-c" style="width: 100%" v-show="allEdit">
        <div v-show="!edit">
          ...
          <div v-else>
            ...
            <div class="m-t-20">
              <el-button
                :disabled="agreeFlag==1"
                :loading="agreeFlag==0"
                @click="agreed(0)">
                拒绝签署
              </el-button>
              <el-button
                :disabled="agreeFlag==0"
                :loading="agreeFlag == 1"
                type="primary"
                @click="agreed(1)">
                同意合同
  ...
  export default {
    ...
    methods: {
      ...
      postContract(num) {
        ...
        if (_.includes(this.editData.design_output, '其他（副）')) {
          if (this.editData.design_output.length == 1) {
            ...
            let otherIndex = _.indexOf(this.editData.design_output, '其他（副）')
            updata.design_output[otherIndex] = this.designOutputOther
          }
        }
      ...
      getContarct() {
        this.apiGet('requires/' + this.id + '/contracts/' + this.conId).then((res) => {
          ...
          if (res.code == 200) {
            ...
            if (!_.includes(this.conDesignType, res.data.design_type) || res.data.design_type == '其他类型') {
            ...
            for (let i = res.data.design_output.length - 1; i >= 0; i--) {
              if (!_.includes(this.conDesignOutput, res.data.design_output[i]) || res.data.design_output[i] == '其他（副）') {
