    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="权限节点" class="roleCheckbox" prop="powers">
        ...
        <el-row v-show='clientShow' class='row-border'>
          ...
          <div v-show='clientSubshow.unDemand' class='row-bordb'>
            <el-form-item label='未关联需求方'>
              <el-checkbox-group v-model="unDemandSel">
                <el-checkbox v-for="ssubitem in clientRole.unDemandRole" v-show='ssubitem.visible == 1' :label="ssubitem.title"></el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </div>
          <div v-show='clientSubshow.unService' class='row-bordb'>
            <el-form-item label='未关联服务方'>
              <el-checkbox-group v-model="unServiceSel">
                <el-checkbox v-for="ssubitem in clientRole.unServiceRole" v-show='ssubitem.visible == 1' :label="ssubitem.title"></el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </div>
    ...
    methods: {
      sortRole() {
        ...
        if (this.unDemandSel.length > 0) {
          clientRoleList = fromRole.pushRole(this.ruleOptions, clientRoleList, this.unDemandSel, 1, 1)
        }
        if (this.unServiceSel.length > 0) {
          clientRoleList = fromRole.pushRole(this.ruleOptions, clientRoleList, this.unServiceSel, 1, 2)
        }
        if (this.mySel.length > 0) {
          clientRoleList = fromRole.pushRole(this.ruleOptions, clientRoleList, this.mySel, 1, 3)
        }
