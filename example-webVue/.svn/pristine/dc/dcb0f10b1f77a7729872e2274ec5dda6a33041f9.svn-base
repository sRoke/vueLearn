...
export default {
  ...
  methods: {
    handleCurrentChange(val) { // 分页的选择
      return this.fetchStaffByRole(val)
    },
    open() { // 打开模态框
      this.fetchRole()
      _(this.followPeople).forEach((item, index) => {
        if (!this.results[`${item.label}`]) {
          this.$set(this.results, `${item.label}`, [])
        }
    ...
    singleAssign() { // 单个分配
      ...
      let followPeople = this.followPeople.map(item => item.id)
      let data = {
        followPeople,
        id: this.clientIds
      }
    ...
    handleTabChange(tabs, event) { // 点击tab选择角色
      this.currentPage = 1
      this.keywords = null
      this.activeRole = this.roleList[~~(tabs.index)]
      this.activeName = this.activeRole.title || this.activeRole.label
      this.fetchStaffByRole()
    },
    fetchStaffByRole(page = 1) { // 根据角色获取人员
      this.loading = true
      let params = {
        page,
      ...
      this.apiGet('customers/customer_follow', { params })
        .then((res) => {
          this.loading = false
          if (res.code !== 200) {
          ...
          this.staffList.push(...res.data.list)
          this.totalCount = res.data.count
        })
    },
    searchData() {
      return this.fetchStaffByRole()
    },
    releaseConfirm(role) { // 确认释放
      ...
      this.$confirm(message, '提示', {
        ...
        this.releaseExec(data, role)
      })
    },
    releaseExec(data, role) {  // 执行释放请求
